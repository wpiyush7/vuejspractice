...
export const generateGameBoard = (): number[][] => {
  ...
  const game: number[][] = Array.from({ length: rowsLength }, () =>
  ...
  const validateAndReturnNum = (
    ...
    const columnCount = game.reduce(
      (count, row) => count + (row[colIndex] !== 0 ? 1 : 0),
      0
    );
